...
from assistant.inline import inline_autobc
from Navy.database import dB
from Navy.helpers import AUTOBC_STATUS, ButtonUtils, Emoji, animate_proses, CMD

from plugins.spambot import spam_bot
...
@CMD.UBOT("autogcast")
@CMD.DEV_CMD("autogcast")
@CMD.FAKEDEV("autogcast")
async def autobc_cmd(client, message):
    ...
    auto_text_vars = await dB.get_var(client.me.id, "AUTO_GCAST") or []
    ...
    if type == "on":
        ...
        await msg.edit(
            f"
<blockquote expandable>**__{em.msg} Status Auto Broadcast:
{em.owner} Status: {status_text}
{em.ket} Jumlah Grup: {total_groups}
{em.speed} Delay: {delay}  detik 
{em.data} Pesan Di Simpan: {len(msgs)} Pesan
{em.proses} Putaran: {rounds} Kali
{em.pin} Terakhir Broadcast: {last_broadcast_time}__**</blockquote>"
        )
...
__MODULES__ = "ᴀᴜᴛᴏɢᴄᴀsᴛ"
__HELP__ = """<blockquote>Command Help **ᴀᴜᴛᴏɢᴄᴀsᴛ**</blockquote>
<blockquote expandable>--**Basic Commands**--

    **Add text for ᴀᴜᴛᴏɢᴄᴀsᴛ**
...
**Note**: please add the text first, before enable ᴀᴜᴛᴏɢᴄᴀsᴛ.</blockquote>
<b>   {1}</b>
"""