...
from pytgcalls.exceptions import NotInCallError, YtDlpError
from config import URL_LOGO
from Navy import dB, navy
from Navy.database import state
from Navy.helpers import (CMD, Emoji, Tools, YoutubeSearch, gen_qthumb, stream,
                          telegram)
from Navy.helpers import ytdlp
from Navy.logger import logger

__MODULES__ = "ᴘʟᴀʏ ᴍᴜsɪᴄ"
...
async def skip_songs(client, chat_id):
    ...
    try:
        ...
        return await pros.edit(
            f"{emo.gagal}<b>Failed to play the next song:</b>\n<code>{str(e)}</code>"
        )
    finally:
...
async def _(client, message):
    ...
    if audio_telegram:
        ...
        if re.match(r"^https?://", gt_txt):
            media_link = gt_txt
        else:
            yt_search = YoutubeSearch(gt_txt, max_results=1)
            await yt_search.fetch_results()

            media_link = clean_yt_url(yt_search.get_link())
            audio_original = media_link
    ...
    if not group_calls:
        ...
        try:
            ...
            logger.error(f"Error Thumb: {str(e)}")
            await message.reply(
    ...
    if str(query) == "disable":
        ...
        if chat and chat.linked_chat:
            ...
            if len(message.comand) < 3:
                return await message.reply(
                    f"{emo.gagal}**Please user command `{message.text.split()[0]} linked` @nikolnewversion"
                )
